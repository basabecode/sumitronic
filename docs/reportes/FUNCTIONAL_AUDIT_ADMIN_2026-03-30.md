...
**Problema:** La función `uploadImage` intenta subir archivos a **Supabase Storage** (`supabase.storage.from(bucket).upload`), pero CLAUDE.md documenta explícitamente: _"Supabase Auth and Storage are NOT active in this environment."_
...
const urls = await Promise.all(
  toUpload.map(
    file => uploadImage(file, 'products').catch(() => '/placeholder.svg') // ← silencia el error
  )
)
```
