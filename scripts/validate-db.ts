...
const TABLES_TO_CHECK = [
  ...
  { name: 'product_variants', required: false, description: 'Variantes de productos (talla/color)' },
];
...
async function validateSchema() {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === '42P01') {
           ...
           results.push({ ...table, status: '⚠️ ACCESO RESTRINGIDO', detail: `Existe, pero error: ${error.message}` });
  ...
  if (allCriticalPassed) {
    console.log('🎉 RESULTADO: La estructura de la Base de Datos PARECE COMPLETA y cumple con los requerimientos funcionales principales.');
